...
# create n models
step(/^(\d+) #{capture_plural_factory} existe(?: com #{capture_fields})?$/) do |count, plural_factory, fields|
  create_models(count, plural_factory.singularize, fields)
  puts RegisteredUser.count
end
...
step /#{capture_model} existe com #{capture_fields}$/ do |model_name, fields|
  user = create_model(model_name, fields)
  puts user.inspect
end
